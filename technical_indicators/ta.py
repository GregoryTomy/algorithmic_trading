...
def bollinger_band(prices, window=21, num_std=1):
    """
...
def slope(prices, window=5):
    ...
    std = (prices - prices.min()) / (prices.max() - prices.min())
...
def renko(data, n=120):
    ...
    ren.brick_size= round(atr(df['close'], df['high'], df['low'], n).iloc[-1], 0)
